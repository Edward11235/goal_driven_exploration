"""
Path planning module (A*): Optimized distance-based obstacle avoidance
"""
import heapq
import math
...
# Precompute penalty patterns once during initialization
_penalty_cache = {}
for dx, dy in product(range(-DISTANCE_THRESHOLD, DISTANCE_THRESHOLD+1), repeat=2):
    distance = abs(dx) + abs(dy)  # Manhattan distance
    if 0 < distance <= DISTANCE_THRESHOLD:
        penalty = OBSTACLE_WEIGHT / (distance ** 1.5)
        _penalty_cache[(dx, dy)] = penalty
...
def astar(start, goal, grid):
    ...
    def get_obstacle_penalty(x, y):
        """Optimized penalty calculation using precomputed patterns"""
        penalty = 0.0
        ...
        # Check only relevant portion of the grid
